    ...
    try {
      ...
      const message =
        err instanceof AxiosError
          ? err.response?.data?.message ?? "Registration failed. Try a different email or check backend status."
  ...
  return (
    <div className="max-w-md mx-auto mt-20 bg-white p-6 rounded shadow">
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <button disabled={isPending} className="w-full bg-slate-900 text-white py-2 rounded">{isPending ? "Creating account..." : "Register"}</button>
      </form>
